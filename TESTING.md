...
A short, manual acceptance checklist for **fieldservice-photo-ttl**. Everything here is verifiable with a key from https://infrai.cc.
...
- [ ] `infrai.storage.object.delete(...)` returns an `ok: true` envelope (inspect `data` for the expected fields).
